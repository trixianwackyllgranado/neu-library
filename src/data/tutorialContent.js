...
const TUTORIAL_CONTENT = {
  ...
  'admin/users': {
    ...
    purpose: 'Manage all registered library users — students, staff, and admins. View profiles, change roles, pre-invite new staff, and block accounts that violate library policy.',
    howItWorks: 'The user list loads from Firestore in real time. Filter by role (All, Visitors, Staff, Admins, Blocked), search by name/email/ID, and click any row to open a profile panel. Role changes and blocks are all logged in the admin audit trail.',
    scenario: 'A student is repeatedly causing disruptions. Open their profile in User Management, scroll to the Danger Zone section, and click "Block Account." They are immediately signed out of the web app and their active library session is force-closed — in real time, even if they are currently logged in on another device.',
    instructions: [
      'Search users by name, email, or student ID.',
      'Use the role tabs to filter — the Blocked tab shows all restricted accounts separately.',
      'Click any user row to open their profile panel.',
      'Under Role: promote visitors to staff or demote staff back to visitor (requires a reason).',
      'Under Edit: update profile info or fix a name error.',
      'Under Danger Zone: click "Block Account" to immediately restrict the user. They are signed out and kicked from any active library session in real time.',
      'Blocked users appear under the Blocked tab and cannot log in or check into the library until you click "Unblock Account."',
      'IT Support accounts (marked with a shield) cannot be demoted or blocked.',
    ],
  },
