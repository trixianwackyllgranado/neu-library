...
// Nav items per role — visitors have no nav (they use kiosk on dashboard directly)
const NAV = {
  visitor: [],
  staff: [
...
export default function AppLayout({ children }) {
      ...
      <nav style={{flex:1,padding:'8px 8px',display:'flex',flexDirection:'column',gap:2,overflowY:'auto'}}>
        ...
        {navItems.map(item => (
          <NavItem key={item.to} to={item.to} label={item.label} icon={item.icon}
            collapsed={collapsed&&!mob} onClick={mob?()=>setMobileOpen(false):undefined} />
        ))}
      </nav>
  ...
  return (
    <div style={{display:'flex',minHeight:'100vh',background:'var(--bg-base)'}}>
      ...
      {/* Sign-out modal when session is active */}
      {showSignOut && (
